...
# def rms_normalize(wav, ref_dBFS=-23.0):
def rms_normalize(wav, ref_linear):
	# RMS normalize
...
def torch_cal_rms(wav):
    return (wav.square().mean() + eps).sqrt()

def torch_rms_normalize(wav, ref_linear):
    rms = torch_cal_rms(wav)
    gain = ref_linear / (rms + eps)
    wav = gain * wav
    return wav

def load_wav(path, max_len=None, sr=None):
