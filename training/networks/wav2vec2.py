...
class Wav2Vec2Wrapper(nn.Module):
    ...
    def __init__(self,
        ...
        # warning can occurs since `Wav2Vec2Model` does not contain quantization modules
        # alias
        self.linguistic = linguistic or Wav2Vec2Wrapper.LINGUISTIC
        self.cache_dir = cache
        self.model = Wav2Vec2Model.from_pretrained(name, cache_dir=self.cache_dir)
        self.channels = self.model.config.output_hidden_size
        self.strides = np.prod(self.model.config.conv_stride).item()
        # feature extractor
        ext = Wav2Vec2FeatureExtractor.from_pretrained(name, cache_dir=self.cache_dir)
        # resampler
        self.sr = sr
