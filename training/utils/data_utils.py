...
## modified 
def load_audio(args, filename, start=None, end=None, max_frames=None, renormalize_volume=False, sample_rate=22050):
    ...
    pytorch_mspec = mel_spectrogram(torch.tensor(audio, dtype=torch.float32).unsqueeze(0), args.data.filter_length, args.data.n_mel_channels, sample_rate, args.data.hop_length, args.data.win_length, 0, 8000, center=False)
...
def get_emg_features(emg_data, debug=False): # frame * 8 -> frame * (8*(5+9))
...
def read_phonemes(textgrid_fname, max_len, scale):
    tg = TextGrid(textgrid_fname)
    phone_ids = np.zeros(int(tg['phones'][-1].xmax*scale)+1, dtype=np.int64)
    phone_ids[:] = -1
    ...
    for interval in tg['phones']:
        ...
        phone_ids[int(interval.xmin*scale):int(interval.xmax*scale)] = ph_id
